...
//!
//! **What it is:** the pure math behind `apelles_timeline::Clip::pan` — turning
...
//!   them into. `apelles_media::audio`'s `LevelEnvelope` multiplies them into
//!   the first two channels of a source's own interleaved buffer;
//!   `@apelles/editor`'s `timelineExportAudio.ts` mirrors this function
...
//! `1/√2 ≈ 0.707` at the centre — the familiar "−3 dB centre" pan law. Apelles
