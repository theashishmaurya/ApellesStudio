...
//! `apelles_timeline::Clip::eq_bands`) plus the pure DSP that turns a band into
...
//! the mixer that consumes it is L1 `apelles-media` and cannot reach up to L2's
//! `Clip`.
//!
...
//! here is an approximation or a Apelles invention. The one design choice this
...
//! parameterisation, `@apelles/editor`'s `timelineExportAudio.ts` compiles each
...
/// high-shelf in the screenshot, and each is switchable. Apelles offers all five
...
/// a set of disabled bands — see `apelles_timeline::Clip::eq_bands`.
...
impl Default for EqBand {
    /// Manual, not derived, for `apelles_timeline::Clip`'s own stated reason
