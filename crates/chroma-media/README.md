...
- **Modules**, one per source file it was extracted from:
  ...
  - `audio` — the Edit-tab playback engine (symphonia → rubato → dasp_sample →
    ...
    D-149, level D-223, EQ D-224), its per-source **speed retime** (D-241 — a
    ...
    deliberately NOT mirrored here, see D-241), the single transport's
...
- **Resolve the timeline.** "Which clip is under this playhead" belongs to
  ...
  *media* fact stated in seconds — which is why D-147's `audio::FadeEnvelope`
  is seconds too, why D-241's `audio::AudioSpeedSegment` is seconds too, and
  ...
  also why D-241's retime resolves a speed ramp it is handed rather than one it
