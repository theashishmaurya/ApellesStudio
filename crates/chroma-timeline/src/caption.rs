...
//! the live preview and `@chroma/editor`'s `timelineExport.ts` for the export.
//!
//! ## Why this module exists at all: the two-engine parity problem
//!
//! Chroma renders every picture twice, in two different engines — the live
...
/// and the box at the rect below; `@chroma/editor`'s export compiler emits a
...
impl CaptionLayout {
    ...
    /// `@chroma/editor`'s `caption.ts` (`captionLayout`) — a single
