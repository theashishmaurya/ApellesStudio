//! Apelles project model (D-037) — the saved project the launcher opens.
//!
//! **Home:** `apelles-project` since D-148 (`docs/notes/crate-extraction-plan.md`
...
//!   (D-141 §1). `chroma::project` is a `pub use apelles_project::*;` shim at
...
//!
//! **Multi-track NLE, Phase A (D-054):** `apelles_timeline::Clip` gained an
...
//! shot strip now reads the active `apelles_timeline::Timeline`'s clips
...
//! operate on `apelles_timeline::Clip` via [`append_media_clip`] instead.
...
/// [`load_manifest`] reads it and [`apelles_timeline::Timeline::normalise_legacy_positions`]
...
/// — `apelles_timeline::Clip` (+ its new `media_id`) is now the single source
...
pub struct ProjectManifest {
    ...
    #[serde(default)]
    pub timelines: Vec<apelles_timeline::Timeline>,
...
/// D-292 — the still-image counterpart of [`MediaVideoInfo`]: everything the
...
pub struct MediaItem {
    ...
    /// D-292 — probed at import time when the source is a **still image**
    ...
    /// `project.json`; this key is simply absent on every pre-D-292 item and
    ...
    /// it is deliberately here rather than on [`apelles_timeline::Clip`]: one
...
/// D-292 — and still images, which are first-class Edit-tab sources; this gate
...
impl MediaItem {
    /// D-292 — has this item ever produced a usable answer from a probe?
    ///
    ...
    /// D-292 — is this pool item a **still image** rather than moving footage?
    ...
    /// D-292 — re-read this item's source and refresh whichever probed-facts
    ...
    /// D-292 each of them spelled `video::probe(..)` inline, which is precisely
...
/// thumbnail cache.
///
/// D-292 — factored out of `generate_media_thumb` when a second producer
...
/// D-292 — [`generate_media_thumb`]'s still counterpart: the image itself,
...
/// D-292 — generate this source's pool thumbnail, whichever kind of source it
...
///
/// **D-292 — two probes, one per kind of source.** This used to call
...
/// [`apelles_media::probe::probe_cached`], which is itself `(mtime, len)`-keyed
...
pub fn refresh_media(
    ...
    for path in paths {
        ...
        // D-292 — the video-vs-still branch is `reprobe_source`'s, not spelled
...
// `ProjectShot` -> `apelles_timeline::Clip` migration: appending a new
...
/// behaviour `@apelles/editor`'s `add_clip` op gives the Edit-tab drag — kept
...
pub fn append_media_clip(
    ...
    let probed = apelles_media::probe::probe_cached(Path::new(&item.source_path)).ok();
...
/// picked") through `apelles_timeline::Timeline::resolve_video_clip_at` at
...
/// `apelles-timeline`'s own `resolve_video_clip_at_matches_single_track_behavior`
...
///
/// Deliberately NOT renamed to `~/Movies/Apelles` by the D-265 product rename:
/// this is an on-disk location that already holds the owner's real projects,
/// and the launcher only lists `*.chroma` bundles found here. Renaming it
/// silently orphans every existing project. It belongs with the rest of the
/// on-disk compatibility set (the `.chroma` extension, the `chroma.grade/N` and
/// `chroma.project/N` schema ids, `~/.chroma/config.json`) — all of which move
/// together, behind a real migration, or not at all. See D-265.
...
/// `apelles_timeline::Timeline::id`'s doc). A project saved after this change
        ...
        Some(m) if m > CURRENT_MAJOR => {
            return Err(format!(
                "project.json is schema '{schema}' — newer than this build supports \
                 (chroma.project/{CURRENT_MAJOR}). Upgrade Apelles."
            ));
        }
...
pub fn new_project_in(
    ...
    // then a `apelles_timeline::Clip` referencing it on a fresh timeline — the
...
mod tests {
    ...
    /// **D-292, end to end at the pool layer — the owner's actual bug.**
    ///
    ...
    /// D-292 — a still whose `project.json` predates the `image` key, and a
    /// still item's own round trip. The wire shape must stay additive: a
    /// pre-D-292 item deserializes with `image: None` and no migration, and a
    ...
    fn migrate_shot_grades_to_clips_migrates_the_clean_1to1_case() {
        ...
        manifest.timelines.push(Timeline {
            ...
            name: "x".into(),
            rate: None,
            tracks: vec![apelles_timeline::Track {
    ...
    /// `apelles-timeline`'s own single-track-behavior guarantee.
    #[test]
