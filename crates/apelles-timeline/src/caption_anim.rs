...
//! consume — `chroma::caption_render` for the live preview, `@apelles/editor`'s
...
/// frame; `@apelles/editor`'s `captionAnim.ts` mirrors it exactly, and the
...
/// looks like. Mirrored by `captionWordColor` in `@apelles/editor`'s
/// `captionAnim.ts`.
///
