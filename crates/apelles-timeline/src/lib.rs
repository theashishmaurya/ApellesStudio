//! # apelles-timeline — L2 domain model (D-039)
//!
...
//! compositing, no media probing. It only depends on `apelles-types`. Callers
...
//! `ensure_audio_track_with_room` here plus `@apelles/editor`'s `add_clip`
...
//! (`chroma::edit`'s compositor, `@apelles/editor`'s ffmpeg export compiler)
...
//! `@apelles/editor`'s ffmpeg compiler are the two consumers that turn that into
...
//! `@apelles/editor`'s ffmpeg compiler turn that into pixels. See D-230 for why
...
//! live in `@apelles/editor`'s `timeline.ts` alongside every other op that
...
/// (`@apelles/editor`'s `captionAnim.ts`) and a reader checking the two against
...
/// mirror of `@apelles/editor`'s `speedRamp.ts`, and a reader checking the two
...
// D-147 — the fade curve math itself lives in `apelles-types` (L0), not here.
...
// for the compositor's opacity) and `apelles_media::audio::FadeEnvelope` (L1,
...
pub use apelles_types::ease::EaseCurve;
pub use apelles_types::fade::{self, fade_gain};
pub use speed_ramp::{SpeedPoint, SpeedSegment};

// D-224 — same shape, same reason, one layer further: a clip's EQ band type
// and the Audio EQ Cookbook biquad math behind it live in `apelles-types` (L0)
// because the mixer that runs the filters is `apelles_media` (L1) and cannot
...
// naming an ffmpeg filter — see `apelles_types::eq`'s own doc for the
// measurement behind that.)
pub use apelles_types::eq::{self, EqBand, EqBandKind};

// D-230 — same shape again: an adjustment clip's five-parameter correction and
// the two-stage colour operator it resolves to live in `apelles-types` (L0)
...
// pixel consumers — `chroma::edit`'s CPU compositor and `@apelles/editor`'s
...
pub use apelles_types::adjustment::{self, AdjustmentLayer, AdjustmentOps};

/// B-079 — mirrors `@apelles/editor/timeline.ts`'s `DEFAULT_FPS`: the project
...
/// already commits to). Exact mirror of `@apelles/editor/timeline.ts`'s
...
/// corresponds to. Mirrors `@apelles/editor/timeline.ts`'s
...
/// GUI's own swatch row is `@apelles/editor/timeline.ts`'s `MARKER_COLORS`, the
...
pub struct Track {
    ...
    ///
    /// This crate never reads it (no media/rendering here — see the module
    /// doc); `apelles_media::audio`'s `DuckEnvelope` is the consumer, built
    ...
    /// `apelles_media::audio`, exactly where the linear/dB boundary belongs.
    ///
...
/// `@apelles/editor`'s `checkTransition` precondition) are what verify the
...
/// `size`/`color`.
///
/// **Single-line only, enforced at the write path** (`@apelles/editor`'s
...
/// D-194 already gave `@apelles/editor`'s `timeline.ts`, chosen again here for
...
/// (every real edit, GUI or MCP, goes through `@apelles/editor`'s `applyOp` +
...
pub struct Clip {
    ...
    /// (`@apelles/editor`'s `clipFromDraggedMedia`), or the Colorist "add to
    ...
    /// `@apelles/editor`'s `editor_add_clip`/`linkedClipsFromDraggedMedia` from
    ...
    /// preview) go through. `@apelles/editor`'s `speedRamp.ts` is the exact
    ...
    /// (`apelles-timeline` is a pure L2 domain crate — see the module doc;
    ...
    /// (`apelles_types::clip_volume`); there is deliberately no ceiling, exactly
    ...
    ///
    /// The law itself is `apelles_types::pan_gains` (constant power, 0 dB
    ...
    /// four-band strip (`DEFAULT_EQ_BANDS` in `@apelles/editor`'s `eq.ts`):
...
impl Clip {
    ...
    /// D-229/D-230 — whether this clip's picture is **generated** rather than
    /// decoded from `source_path`: a title, a caption, or an adjustment clip
    /// (which has no picture of its own at all).
    ///
    ...
    /// `is_text()` that actually meant "generated" — which is exactly why
    /// D-230's adjustment clip (also `source_path`-empty) folds in here too
    /// rather than needing its own parallel check at every media-open site.
    pub fn is_generated(&self) -> bool {
        self.is_text() || self.is_caption() || self.is_adjustment()
    }
    ...
    /// edit, GUI or MCP, goes through `@apelles/editor`'s `applyOp` +
    ...
    /// exactly `@apelles/editor/timeline.ts`'s `endFrame` (B-077/D-194).
    ...
    /// arithmetic bit-for-bit unchanged. Mirrors `@apelles/editor`'s
    ...
    /// defence, not the feature.** `@apelles/editor`'s `checkTransition` refuses
...
/// not bookkeeping.** `apelles_media::decode_pipe` keys one live `ffmpeg` process
...
impl Timeline {
    ...
    /// Exact mirror of `@apelles/editor/timeline.ts`'s `timelineFps` — the one
...
impl Track {
    ...
    /// (`@apelles/editor`) refuses to write two transitions whose windows
...
mod tests {
    ...
    /// B-075/B-077 on the very first save after this pass: `@apelles/editor`
    ...
    /// contract lives in `@apelles/editor`'s `applyOp`, see `markersOf`). Pinned
    ...
    fn clip_json_without_audio_fields_loads_at_unity_centre() {
        ...
        assert_eq!(apelles_types::pan_gains(c.pan), (1.0, 1.0));
        assert_eq!(apelles_types::clip_volume(c.volume), 1.0);
    }
    ...
    // Same repro numbers/style as `@apelles/editor/timeline.test.ts`'s own
    ...
    mod b079_mixed_native_fps {
        ...
        fn timeline_fps_reads_the_rate_field_and_falls_back_to_default() {
            ...
            t.rate = Some(apelles_types::Rational { num: 30, den: 1 });
            assert_eq!(t.fps(), 30.0);
            t.rate = Some(apelles_types::Rational { num: 0, den: 1 });
            assert_eq!(
                t.fps(),
                DEFAULT_FPS,
