...
- **Deps:** `serde`, `serde_json`, `base64`. **Zero fork deps** — no
  `chroma-types`, no `wgpu`, no `ffmpeg`, no store. The real code never
  ...
  `chroma-types` edge) and corrected it rather than adding an unused
  dependency to match a stale doc.
- **Model vs renderer:** this crate is the *document*. `chroma-grade`
  (future, L1) is the *renderer* that links the RapidRAW shader. Same split
  as `chroma-timeline` vs `chroma-compositor`.
