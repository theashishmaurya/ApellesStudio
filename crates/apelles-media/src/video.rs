...
///
/// `resolution` is `apelles_types::Resolution` (D-053) via `#[serde(flatten)]`
...
/// importable source" gate (`apelles_project::media_item_is_online`,
...
///
/// **D-281 — video OR audio OR STILL.** A still image failed this gate for
...
pub fn probe(path: &Path) -> Result<VideoInfo> {
    ...
    Ok(VideoInfo {
        resolution: apelles_types::Resolution::new(width, height),
        fps_num,
        fps_den,
        duration_secs,
        frame_count,
...
/// bookkeeping" contract `crates/apelles-timeline`'s own `DEFAULT_FPS = 24.0`
...
fn probe_audio_only(path: &Path, fmt: Option<&serde_json::Value>) -> Result<VideoInfo> {
    ...
    Ok(VideoInfo {
        resolution: apelles_types::Resolution::new(0, 0),
...
/// `n`" as the rest of Apelles defines it. On a VFR source it previously meant
...
mod tests {
    ...
    /// **D-281, the extension-gate half.** A still is importable media too —
