//! # apelles-media — L1 media I/O (D-146, `docs/notes/crate-extraction-plan.md` §2.2)
//!
//! **What it is:** everything Apelles knows about *a media file as a source of
//! frames and samples*, with no opinion at all about timelines, projects,
//! grades or the GUI. It is the layer beneath `apelles-project` /
//! `apelles-timeline` and above `apelles-types`.
//!
...
//!   stream (D-278), the DSP behind the Edit tab's playback-RATE control. Used
...
//!   `apelles-timeline` + the app's `chroma::edit` today and
//!   `apelles-compositor` later. This crate is handed a path, a source second
