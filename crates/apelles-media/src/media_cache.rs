//! Apelles' persistent, disk-backed media cache (D-128).
//!
...
//! What it does: resolves a cache root once (from `app/src-tauri`'s `lib.rs`
//!   `setup`, which calls [`init`] with the same
...
//!   belongs to its own module ([`crate::video`], and `crate::filmstrip` /
//!   `crate::audio` once those land).
...
//!
//! **Why this exists (D-128).** Every cache Apelles had before this module was
...
///
/// Behind the `test-support` feature since D-146 rather than `#[cfg(test)]`:
/// `app/src-tauri`'s own tests bind a `tempdir` root through this, and a
/// separate crate's `#[cfg(test)]` items are unreachable from there.
/// `app/src-tauri` enables the feature from its `[dev-dependencies]` only, so
/// a release build of the app links none of it.
