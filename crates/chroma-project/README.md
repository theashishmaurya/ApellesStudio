...
- **Deps:** `chroma-types` (`Resolution`), `chroma-timeline` (`Timeline` /
  `Clip` / `Track`), **`chroma-media`** (probe + thumbnail extraction),
...
## The `chroma-project → chroma-media` edge
...
`probe::probe_cached` — which after D-146 is `chroma-media`. The edge is
...
doc's table is corrected. There is **no** `chroma-grade-model` edge: this
...
and `chroma_media::probe` each carry their own copy — and consolidating all
four into one `chroma-media` test fixture is flagged in the extraction plan's
