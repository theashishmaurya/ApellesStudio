//! # chroma-project — L2 the `.chroma` project document (D-148, `docs/notes/crate-extraction-plan.md` §2.3)
//!
...
//! lifecycle that turns a manifest into an active [`chroma_timeline::Timeline`].
//!
//! Modules:
//! - [`manifest`] — the model itself, moved verbatim from
//!   `app/src-tauri/src/chroma/project.rs` L1–1638. Re-exported at the crate
//!   root (`chroma_project::ProjectManifest`, …) because it *is* the crate;
...
//!   probing go through [`chroma_media`]; grade *files* are moved around by
