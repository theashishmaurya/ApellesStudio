//! # apelles-grade-model — L2 domain model (D-025, extracted D-143)
//!
//! **What it is:** the on-disk `grade.json` document (D-025) — save, load, and
//! the versioned schema migration gate — moved verbatim out of
//! `app/src-tauri/src/chroma/grade.rs`. v1 is a **versioned, documented
//! wrapper** around RapidRAW's live `adjustments` blob plus shot context,
//! with every mask matte externalised to a sidecar file so the JSON stays
//! diff-able. It is deliberately NOT doc 06's aspirational ordered `stack`
//! (that re-model pairs with the v2 node graph, D-005). See D-025.
//!
//! **What it does:** [`save_grade`] takes the assembled grade the frontend
//! sends (which holds the live `adjustments` — D-020), writes the
//! `<name>.mattes/` PNGs, rewrites each mask param to a `{"$matte": …}` /
//! `{"$trackDir": …}` / `{"$depthDir": …}` reference, and pretty-prints
//! `grade.json`. [`load_grade`] reverses it and runs a schema migration
//! (identity past v1; hard-errors a newer major).
//!
...
//! does not copy a tracked-matte folder — only references it (D-019). No
//! typed `adjustments`/mask schema: the document is intentionally untyped
//! `serde_json::Value` past the `schema`/`shot`/`notes` envelope, because the
//! canonical shape is owned by the frontend `useEditorStore` (D-020) and a
//! typed Rust mirror would just drift — see D-025's own reasoning for
//! rejecting the ordered `stack` in v1. (This supersedes this crate's
//! original D-039-step-1 skeleton, a placeholder typed `Grade`/`ShotRef`
//! pair that predated this real extraction and was never wired to any
//! caller — replaced outright rather than kept alongside real code.)
//!
//! **Model, not renderer.** This crate is the *document*: parse, validate,
//! migrate, serialise. The crate that turns it into pixels is `apelles-grade`
//! (a future L1 crate that links the RapidRAW shader). The split mirrors
//! `apelles-timeline` (model) vs `apelles-compositor` (renderer).
//!
//! **Status:** real extraction, D-143 — see `docs/notes/crate-extraction-plan.md`
//! §2.4. The 2 `#[tauri::command]` wrappers (`chroma_save_grade` /
//! `chroma_load_grade`) stay in `app/src-tauri/src/chroma/grade.rs` per the
//! "commands do not move" rule (D-141) and just call into this crate.
...
/// breaking change; [`load_grade`] rejects a file with a newer major.
...
pub const MATTE_KEYS: [&str; 2] = ["maskDataBase64", "mask_data_base64"];
...
pub fn save_grade(path: &str, grade: Value) -> Result<SaveResult, String> {
...
pub fn load_grade(path: &str) -> Result<Value, String> {
        ...
        Some(m) if m > CURRENT_MAJOR => {
            return Err(format!(
                "grade.json is schema '{schema}' — newer than this build supports \
                 (chroma.grade/{CURRENT_MAJOR}). Upgrade Apelles."
            ));
        }
