...
//! *renderer's* job — for Chroma that is the Colorist's own wgpu pipeline,
...
/// The lattice edge Chroma bakes at. 33 is the industry-default `.cube` size
...
mod tests {
    ...
    fn cube_text_is_the_iridas_shape_red_fastest() {
        ...
        let text = lut.to_cube_text("Chroma primary grade");
        let lines: Vec<&str> = text.lines().collect();
        assert_eq!(lines[0], "TITLE \"Chroma primary grade\"");
