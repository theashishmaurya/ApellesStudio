...
| L2 | `chroma-timeline` | **yes, real** (D-041/045/046/082/136/137/138/235) | OTIO-shaped edit model: tracks / clips / gaps / ripple / roll / slip / slide, plus the `speed_ramp` time remap the live preview decodes with (D-235, mirrored by `@chroma/editor`'s `speedRamp.ts`, which the exporter compiles from). **Pure.** | types |
