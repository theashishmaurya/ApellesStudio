...
EDITOR_CAPABILITIES: dict[str, Any] = {
    ...
    "still_images": {
        "a_still_is_an_ordinary_clip": (
            "D-281: a STILL IMAGE (.png/.jpg/.jpeg/.webp/.tif/.tiff/.bmp) is a "
...
def editor_get_capabilities() -> str:
    ...
    footage (D-281 — it has no length, so one is synthesized), and known rough
    edges worth testing
...
def editor_import_media(paths: list[str], folder: str | None = None) -> str:
    ...
    STILL IMAGES are importable too (D-281): `.png`, `.jpg`/`.jpeg`, `.webp`,
...
def editor_list_media() -> str:
    ...
    **`video` vs `image` (D-281).** A row has exactly one of them. `video` is
...
def editor_add_clip(
    ...
    STILL IMAGES (D-281). A pool item that is a still (PNG/JPEG/TIFF/WebP/BMP —
...
def editor_swap_clip_media(
    ...
    The new source may be a STILL IMAGE (D-281), and swapping TO one never
