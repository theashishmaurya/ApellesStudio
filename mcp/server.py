...
def editor_text_fonts() -> str:
    ...
    entries yourself (D-239): `editor_add_text_clip`/`editor_set_text_clip`/
...
def editor_add_text_clip(
    ...
    **A title is an ordinary clip on an ordinary VIDEO track**, not a special
    ...
    catalogue key is which (D-239) — pass either on top of any `font`
...
def editor_set_text_clip(
    ...
    `bold`/`italic` (D-239) toggle weight/slant against whatever `font` ends
...
def editor_import_subtitles(
    ...
    **TTML/XML/embedded-MXF subtitles are NOT supported** (D-228). That is a
    ...
    `bold`/`italic` (D-239) toggle weight/slant against `font` (or the
...
def editor_set_caption_style(
    ...
    - `font` — a catalogue key from `editor_text_fonts` ("sans-bold", …), not a
      ...
      read the same file.
    - `bold` / `italic` (D-239) — toggle weight/slant against whatever `font`
