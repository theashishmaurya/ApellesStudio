...
describe('the build emits the expected pages', () => {
  /**
   * D-288: `/docs/mcp/` is deliberately not in this list any more — the page
  ...
  it('does not build /docs/mcp/ while it is hidden (D-288)', () => {
    const routes = pages.map((p) => p.route);
  ...
  /**
   * D-287: the hero's own eyebrow line naming this objection was removed
...
describe('the design system’s one hard rule holds on every built page', () => {
  /**
   * D-284: the signature moved from a scarce rose to ink, but "exactly one
...
describe('no broken internal links', () => {
  ...
  /**
   * D-288: Demo.astro is not rendered on any page right now (the owner's own
   ...
   */
  it('ships no screenshot that nothing references, except Demo.astro’s own (dormant, D-288)', () => {
