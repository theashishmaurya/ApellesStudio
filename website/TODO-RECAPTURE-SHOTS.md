...
**Status: open, and now hidden rather than shown-with-disclosure (D-279).**
...
D-264 through D-278 showed them anyway, disclosed: a caption under each image
said the captures predated the rename and would be retaken rather than
retouched. D-279 changed that — the owner's own call — to simply not showing
them for now:
...
4. Re-enable `Demo.astro` on `index.astro` (re-add the import and the
   `<Demo />` line — D-279 removed both, but the component itself is
