...
now** (D-279) — the Edit captures that do exist predate the rename
...
added D-279) that `astro build` emits at `/sitemap-index.xml`. (An earlier
placeholder, `apelles.video`, was left in this config after the real domain
was bought — D-279 caught and fixed the drift.) The build itself is a plain
