...
fn main(@builtin(global_invocation_id) id: vec3<u32>) {
    ...
    // --- Per-mask blur (D-027) ----------------------------------------------------
    // Defocus the masked region by blending toward `structure_blurred` — the
    // ~40px*scale gaussian of the input that dehaze / glow / structure already
    // compute every render. The sample is the *ungraded* input (a shared pre-pass,
    // not a post-grade blur), so under a strong per-mask grade the defocused area
    // carries a little less of that grade — acceptable for a v1 background defocus
    // (completes depth-haze); a variable-radius post-grade blur would need its own
    // pass. Mixed in linear light, before tone-mapping, weighted by mask * blur/100.
    for (var i = 0u; i < adjustments.mask_count; i = i + 1u) {
        ...
        if (mask_blur > 0.0) {
            ...
            if (blur_w > 0.001) {
                var blurred_lin: vec3<f32>;
                if (is_raw == 1u) {
                    blurred_lin = max(structure_blurred, vec3<f32>(0.0));
                } else {
                    blurred_lin = srgb_to_linear(clamp(structure_blurred, vec3<f32>(0.0), vec3<f32>(1.0)));
                }
