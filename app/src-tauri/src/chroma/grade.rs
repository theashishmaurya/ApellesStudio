...
//! (roadmap "Now" item 5, D-025; the model extracted to `chroma-grade-model`
//! in D-143).
//!
...
//!   `chroma-grade-model` but these thin wrappers stay here.
//! What it does: `chroma_save_grade` / `chroma_load_grade` call straight
//!   into `chroma_grade_model::save_grade` / `load_grade` and translate the
...
//!   or schema migration — all of that is `chroma-grade-model` now. See that
...
pub use chroma_grade_model::SaveResult;
...
pub async fn chroma_save_grade(path: String, grade: Value) -> Result<SaveResult, String> {
    chroma_grade_model::save_grade(&path, grade)
}
...
pub async fn chroma_load_grade(path: String) -> Result<Value, String> {
    chroma_grade_model::load_grade(&path)
}
