...
//!
//! **Two resolvers, deliberately (D-205).** [`interpolate`] above is the
...
//! for the Edit tab's per-property clip-transform keyframes (D-205), where
//! different params really are keyed at different frames and the hold rule
//! would turn a linear ramp into a step (B-092). See that function's own doc.
...
/// counterpart to [`interpolate`], added by D-205 (B-092).
///
...
/// exactly what per-property keyframing (D-205) produces: a `scale` keyed at 0
...
mod tests {
    ...
    // --- D-205 / B-092: the per-param resolver -------------------------- //
    ...
    /// pre-D-205 whole-clip keyframe shape), the two resolvers agree exactly.
    #[test]
