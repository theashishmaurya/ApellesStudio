...
//!   [`super::video::VideoInfo::has_audio`]) **plus** (D-056, Phase C — see
...
//! What it does NOT do: pan/stereo-positioning per track (D-056 scoped this
...
//! `PreviewPane.tsx`).
//!
//! ## Multi-track mixing (D-056, Phase C of `docs/notes/multi-track-nle.md`)
//!
...
/// Smooth soft-knee limiter (D-056, Phase C mixing) — `tanh(x)`. Guarantees
...
/// scaled by its own `gains[i]` first (D-056's per-track gain).
///
/// **Headroom approach — chosen and justified (D-056):** a track at `gain ==
...
///   things exactly true, not just approximately: (1) the pre-D-056
...
/// One audio source for a play session (D-056) — a source path, the source
...
/// lookup the video preview uses) **plus** (D-056, Phase C) any clip on a
...
/// One already-open, mid-decode audio source (D-056) — a `symphonia`
...
/// `sources` (D-056: the baseline video-embedded audio plus any overlapping
...
/// doc for why this keeps the pre-D-056 single-track case byte-identical),
...
mod tests {
    ...
    // --- D-056: the mixer's headroom math (soft_limit / mix_sources) --------
    ...
    /// D-056: [`open_test_project`], optionally with a second, genuine
    ...
    /// End-to-end through the real command surface (D-056): a genuine
