//! Re-export shim for `chroma_media::video` (D-146,
...
//! `crates/chroma-media/src/video.rs`. The `pub use` glob is deliberate and
//! temporary — the plan's wave-4 sweep retargets every call site at
//! `chroma_media::video::…` and deletes this file.
//!
...
//! `is_video_file` — all `chroma_media::video`'s now.

pub use chroma_media::video::*;

#[cfg(test)]
mod tests {
    /// The fork's own routing hook, not `chroma_media::video`'s behaviour —
