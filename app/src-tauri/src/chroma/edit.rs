...
fn resolve_clip_transform_unfaded(clip: &Clip, source_frame: i64) -> ClipTransform {
    ...
    // D-205 (B-092) — resolve **one param at a time, over only the keys that
    ...
    // carry the same params — i.e. every pre-D-205 whole-clip key.
...
mod composite_tests {
    ...
    /// **B-092 / D-205 — per-property keyframes really are independent.**
    ...
    /// The other half of B-092: a param that NO key defines falls back to the
