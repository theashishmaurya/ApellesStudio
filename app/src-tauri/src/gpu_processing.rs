...
pub fn process_and_get_dynamic_image(
    ...
    process_and_get_dynamic_image_inner(
        context,
        &crate::render_core::RenderCaches {
            gpu_processor: &state.gpu_processor,
            gpu_image_cache: &state.gpu_image_cache,
        },
        base_image,
        transform_hash,
        request,
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn process_and_get_dynamic_image_inner(
    context: &GpuContext,
    caches: &crate::render_core::RenderCaches<'_>,
    base_image: &DynamicImage,
    transform_hash: u64,
    request: RenderRequest,
    caller_id: &str,
    ...
    let mut processor_lock = match caches.gpu_processor.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
    ...
    let mut cache_lock = match caches.gpu_image_cache.lock() {
        Ok(guard) => guard,
        Err(poisoned) => {
