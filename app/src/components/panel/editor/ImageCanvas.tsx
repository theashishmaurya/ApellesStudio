      ...
      (id: string | null, data: any) => {
        if (id && data?.parameters && !data.parameters.chromaTrackDir) {
          const container = (adjustments.masks || []).find((c: MaskContainer) =>
            (c.subMasks || []).some((s: SubMask) => s.id === id),
          );
          const sub = container?.subMasks?.find((s: SubMask) => s.id === id);
          if (sub && parseKeyframes(sub.parameters).length > 0) {
            const geometry = snapshotGeometry(sub.type, data.parameters);
            const nextParams = upsertKeyframe(
              { ...sub.parameters, ...data.parameters },
              chromaFrame,
              geometry,
            );
            updateSubMask(id, { ...data, parameters: nextParams });
            return;
          }
        }
        updateSubMask(id, data);
      },
      [adjustments.masks, chromaFrame, updateSubMask],
    );
                    ...
                    <Group x={groupOffsetX} y={groupOffsetY}>
                      {(isMasking || isAiEditing) &&
                        ...
                        sortedSubMasks.map((subMask: SubMask) => {
                          ...
                          const renderSubMask =
                            effParams === baseSubMask.parameters
                              ? baseSubMask
                              : { ...baseSubMask, parameters: effParams };
                          ...
                          return (
                            <MaskOverlay
                              ...
                              onUpdate={updateSubMaskKeyframeAware}
