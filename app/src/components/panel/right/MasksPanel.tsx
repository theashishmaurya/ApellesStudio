...
export default function MasksPanel() {
  ...
  const chromaIsVideo = useChromaStore((s) => s.videoInfo?.isVideo ?? false);
  const chromaTrackProgress = useChromaStore((s) => s.trackProgress);
  const chromaDepthTrackProgress = useChromaStore((s) => s.depthTrackProgress);
                ...
                {!adjustments.masks || adjustments.masks.length === 0 ? (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full mb-6 px-3 py-2 rounded-md text-sm bg-card-active/60 hover:bg-card-active text-text-secondary hover:text-text-primary transition-colors flex items-center justify-center gap-2"
                    >
                      <BringToFront size={15} />
                      {t('editor.masks.addDepthHaze', 'Add depth haze')}
                    </button>
                    <Text variant={TextVariants.heading} className="mb-2">
                ...
                {isSettingsPanelEverOpened && (
                  ...
                  >
                    ...
                    <SettingsPanel
                      ...
                      handleTrackSubject={handleTrackSubject}
                      handleTrackDepth={handleTrackDepth}
                      chromaIsVideo={chromaIsVideo}
                      chromaTrackProgress={chromaTrackProgress}
                      chromaDepthTrackProgress={chromaDepthTrackProgress}
                    />
                  </motion.div>
                )}
              </AnimatePresence>
        ...
        <div className="space-y-4 pt-2">
          ...
          {isComponentMode && (
            <>
              ...
              {activeSubMask.type === Mask.AiSubject && chromaIsVideo && (() => {
                const isTracked = !!(activeSubMask.parameters as any)?.chromaTrackDir;
                return (
                  <div className="flex flex-col gap-2">
                    <button
                      type="button"
                      disabled={!!chromaTrackProgress}
                      onClick={() => handleTrackSubject?.(activeSubMask.id, 'fast')}
                      className="w-full p-2.5 rounded-md border border-surface bg-card-active hover:bg-surface disabled:opacity-60 text-sm font-medium flex items-center justify-center gap-2"
                    >
                      {chromaTrackProgress
                        ? `Tracking… ${chromaTrackProgress.done}/${chromaTrackProgress.total || '?'}`
                        : isTracked
                          ? 'Re-track subject across clip'
                          : 'Track subject across clip'}
                    </button>
                    {isTracked && !chromaTrackProgress && (
                      <button
                        type="button"
                        onClick={() => handleTrackSubject?.(activeSubMask.id, 'quality')}
                        className="w-full p-2 rounded-md border border-surface hover:bg-surface text-xs text-secondary flex items-center justify-center gap-2"
                      >
                        Finalize matte — full quality (before export)
                      </button>
                    )}
                  </div>
                );
              ...
              {activeSubMask.type === Mask.AiDepth && chromaIsVideo && (() => {
                const isTracked = !!(activeSubMask.parameters as any)?.chromaDepthDir;
                return (
                  <button
                    type="button"
                    disabled={!!chromaDepthTrackProgress}
                    onClick={() => handleTrackDepth?.(activeSubMask.id)}
                    className="w-full p-2.5 rounded-md border border-surface bg-card-active hover:bg-surface disabled:opacity-60 text-sm font-medium flex items-center justify-center gap-2"
                    title="Precompute a temporally-consistent depth map for every frame (Video Depth Anything) — the haze then tracks a moving camera without flicker"
                  >
                    {chromaDepthTrackProgress
                      ? `Tracking depth… ${chromaDepthTrackProgress.done}/${chromaDepthTrackProgress.total || '?'}`
                      : isTracked
                        ? 'Re-track depth over clip'
                        : 'Track depth over clip'}
                  </button>
                );
              })()}
