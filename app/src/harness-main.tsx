 ...
 * **D-202 (B-085) — `'preview'` mode now covers canvas click-to-select too.**
...
/** D-202 — per-source natural footprints, keyed by `Clip.source_path`.
 *
 ...
 *  entirely) could never produce. Empty by default, so every pre-D-202
...
function installInvokeStub(): void {
  const handlers: Record<string, (args: unknown) => unknown> = {
    ...
    // D-202 — now resolves its ARGUMENTS (the real command takes track/clip
...
(window as unknown as { __chromaHarness: unknown }).__chromaHarness = {
  ...
  /** D-202 — give one source its own natural footprint (see
