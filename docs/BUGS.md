...
status: open · severity: medium (wrong duration / frame count / resolution / fps everywhere downstream; recovery is an app restart, with no indication that is what is needed) · area: `app/src-tauri/src/chroma/edit.rs` L93–128 (`PROBE_CACHE`, `probe_cached`)
...
- **fix (not yet applied):** validate on hit. `probe_cached` already computes `media_cache::source_key(path)` on the miss path — compute it first instead, and key `PROBE_CACHE` on `(PathBuf, source_key)` (or store the key beside the `VideoInfo` and drop the entry when it differs). That costs one `stat` per hit, the same order as the `HashMap` lookup it guards and orders of magnitude below the `ffprobe` spawn it avoids, and it makes the memory layer honour the identical staleness contract as the disk layer instead of shadowing it. Do this **inside** the `chroma-media` commit-2 move (`docs/notes/crate-extraction-plan.md` §2.2) rather than after — that commit is already rewriting this function's home and signature.
