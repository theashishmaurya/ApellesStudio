...
status: open · severity: low (a real engine capability is unreachable from a manifest; the fallback is the sensible `design.ease.inOut`, so nothing looks broken — it just cannot be changed) · area: `packages/motion-engine/src/engine/schema.ts` L32–37 (`cam2dKey`)
...
- **cause:** the sibling `layer` schema *is* `.passthrough()` (`schema.ts` L64) precisely so per-primitive props survive without being re-declared centrally — the author knew about the distinction and applied it one object over, but not to the camera keys. `cam3dKey` (L39–43) has the same strict shape and, since `Scene3D` has no `ease` concept, the same treatment is harmless there.
- **fix (proposed, not applied — deliberately out of scope for D-151's audit + catalog pass):** declare `ease` on `cam2dKey` as an optional `z.tuple([z.number(),z.number(),z.number(),z.number()])` rather than reaching for `.passthrough()`, so the field is real, typed, and visible to `propCatalog.ts`'s `CAM2D_KEY_FIELDS` — which can then expose it as a real control in the Inspector's existing camera keyframe editor. Passthrough would also work but would silently accept typos, which is exactly what the strict object was buying.
- **why not fixed here:** it is a schema change plus an Inspector field plus a test, and it belongs with the "easing curves on camera keyframes" item the audit lists at priority 6 — not smuggled into a pass the owner scoped as "map out where we lag" plus a Catalog panel.
