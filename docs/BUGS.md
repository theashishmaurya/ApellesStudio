...
status: open · severity: low (a bounded leak of small entries; no user-visible symptom found) · area: `app/src-tauri/src/chroma/filmstrip.rs` (`load_chunk`)
...
- **fix (not yet applied):** make the removal unconditional — a guard type whose `Drop` removes the entry, or the `remove` in a small wrapper around the fallible body. Not a cleanup on the success path only.
