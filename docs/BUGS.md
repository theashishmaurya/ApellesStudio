...
## B-007 — `cargo test chroma::` cross-test pollution: an export test's leftover "loaded video" fails a later, unrelated relight test
...
- **not a regression from D-049:** confirmed by reproducing with only `chroma::export::`/`chroma::relight::` selected, zero `chroma::audio::` tests in the run. D-049's own new tests (`chroma::audio::tests::*`) touch only `state::PROJECT` (via `state::set_project`, always paired with a `set_project(None)` cleanup) — never `state::SESSION`/`set_current_video` — so they neither cause nor are affected by this.
