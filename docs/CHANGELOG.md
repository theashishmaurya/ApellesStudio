...
- **2026-09-03** — **Global undo/redo (D-051): shell-level Cmd/Ctrl+Z spanning all 3
  tabs.** New `@chroma/history` package (a generic `{tab, label, undo(), redo(), ts}`
  stack — a new leaf package, not folded into `@chroma/bridge`, see D-051). Colorist's
  ...
  is always visible (the real UX call, reasoning in D-051). Colorist's own local
