...
## D-049 — Editor timeline audio playback: `symphonia`→`rubato`→`dasp_sample`→`cpal`, a dedicated Rust audio thread that starts in lockstep with the video playhead rather than being driven by it
...
- **Verified.** `cargo test -p RapidRAW chroma::` (no env vars — the default,
  ...
  **Found live while wiring up verification, logged, not fixed (out of
  scope):** `docs/BUGS.md` B-007 — a pre-existing test-isolation gap
  ...
  D-049 code involved, confirmed unrelated to this change.
