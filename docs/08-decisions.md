...
## D-056 — Sources panel fixes: async media commands (B-012), real poster-frame thumbnails, real "New Folder"
...
- **Context.** D-046's own verification was accessibility-driven (`osascript`/System
  ...
  file dialog (**B-012**, see `docs/BUGS.md`), every card shows the same
...
- **1. `chroma_media_list`/`_import`/`_move` → `async fn` (B-012).** Root
  cause and fix are B-012's entry in `docs/BUGS.md` — not repeated here. One
...
- **2. Real poster-frame thumbnails — reuse `video::extract_thumb`, generate
  ...
  - **Scope: generated at import time only, no backfill for pre-D-056
...
- **Verified (shared across all three).** `cargo test chroma::` 114/114
  ...
  - **Real boot + real click-through**, `npm run tauri:dev` (port `15420`,
    ...
    - **Import dialog timing (item 1 / B-012) — honest result, not
      ...
      **definitive, source-level root-cause finding** in B-012's `docs/BUGS.md`
