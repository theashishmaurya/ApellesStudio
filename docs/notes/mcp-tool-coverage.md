...
## Edit tab / multi-track NLE — CLOSED, 48 tools (D-183, 2026-09-07; +1, D-191; +2, D-195; +1, D-196; +3, D-211; +1, D-214; +1, D-216; +1, D-218; +4, D-222; +1, D-223; +1, D-224; +4, D-226; +5, D-229; +2, D-230; +2, D-232; +1, D-234)

> **Count re-verified 2026-09-08 (D-234) against `mcp/server.py` itself**, which is
> what this heading claims to be counting: 52 `@mcp.tool()` functions named
> `editor_*`, minus the 4 media-understanding ones that have their own section
> below, is **48**. The heading previously read 48 while the real total was 47 —
> D-232's two tools were added to the table without bumping it, and the number was
> already one high before that. Both are corrected here rather than carried
> forward. What is verified is the **total** (48, counted from `server.py`) and
> D-232's and D-234's own `+N` entries; the earlier `+N` entries are left as the
> historical log their authors wrote and have not been re-audited one by one, so
> treat the total as authoritative and the breakdown as provenance.
