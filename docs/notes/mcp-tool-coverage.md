...
## Edit tab / multi-track NLE — CLOSED, 68 tools (D-183, 2026-09-07; +1, D-191; +2, D-195; +1, D-196; +3, D-211; +1, D-214; +1, D-216; +1, D-218; +4, D-222; +1, D-223; +1, D-224; +4, D-226; +5, D-229; +2, D-230; +2, D-232; +2, D-233; +1, D-234; +1, D-236; +1, D-238; +1, D-239; +2, D-243; +1, D-256; +6, D-266; +1, D-280)
...
> | `editor_remove_media` (2026-09-07, roadmap item 23) | Remove one or more items from the media pool by id. **For a merely stale or never-probed item, reach for `editor_reprobe_media` first** (B-073/D-270) — that repairs it in place, keeping its id and every reference to it; remove is for an item you genuinely want gone. Wraps the same `chroma_media_remove` the GUI's Sources panel already used; a clip on the timeline still referencing a removed item keeps playing/exporting fine (`Clip.source_path` is an independent copy, never re-read from the pool), only its `media_id` back-link goes stale — reported back in `stillReferencedBy`. |
> | `editor_reprobe_media` (B-073/D-270, 2026-09-09) | Re-probe pool items **by id** — the repair action for an item `editor_list_media` reports as `usable: false` (its first probe failed, so it has no frame count and `editor_add_clip` refuses it). Fix the file problem, then call this with the ids from `unusable`; the item keeps its id, so every reference to it survives, unlike remove-and-re-import. On demand only — a listing never re-probes behind your back, since a re-probe is a real `ffprobe`. Check the response's `stillUnusable` rather than assuming it worked: a source that is still missing comes back honestly unusable. |
...
> | `editor_list_markers` / `editor_add_marker` / `editor_set_marker` / `editor_remove_marker` (D-222, 2026-09-08, roadmap item 27) | Timeline markers — colour-coded, optionally titled flags pinned to a TIMELINE frame. No `track` argument on any of the four, deliberately: a marker belongs to the timeline, not to a clip or a track, so it survives the clip beneath it being trimmed, moved or deleted (`apelles_timeline::Timeline::markers`). `editor_add_marker`'s `frame` defaults to the playhead, matching the GUI's Marker button / `M` shortcut exactly; `color` takes a palette NAME (Resolve's own sixteen — `editor_list_markers` returns the live list) or a raw hex. `editor_set_marker` is a PATCH (an omitted field keeps its value; `""` clears a name/note), and an unrecognised colour is a real error rather than a silent no-change. `editor_get_timeline` reports the same frame-sorted list under its own `markers` key. **Undoable and persisted**, unlike `editor_set_selection`/`editor_set_preview_zoom` below — a marker is document content, not view state. |
...
> | `editor_get_grade_status` (D-256, 2026-09-09) | Read-only. Which clips on the Edit timeline carry a real **Colorist grade** (`clips` of `{clipId, name, graded}`, plus `gradedCount`), and what that grade loses on the way there (`warnings`). The AI half of a feature whose human half is simply that the picture changes: as of D-256 a Colorist grade renders in the Edit preview AND the export automatically — no toggle, nothing to switch on — so what an agent needs is not a mirrored control but the ability to SEE the state a human sees by looking. Three things it answers that nothing else could: (1) whether a grade landed on the clip you meant — the link is the **CLIP id** (D-070), not the source file, so two clips cut from one file grade independently and splitting a graded clip does not copy the grade onto the new half; (2) `graded: false` covering both "never graded" and "graded then reset", which are the same thing to both renderers (no LUT is applied at all, so the pixels are identical to a build without the feature); (3) **`warnings` — read this before trusting an export to match the Colorist tab.** The grade crosses over as a baked 33³ 3D LUT, which carries the whole global grade exactly and cannot carry anything spatial: a mask/local layer, the Colorist crop, depth relight. Each drop is listed here by clip, and an empty list means the exported file is the grade in full. Cheap to call repeatedly — each lattice is memoised on its grade file's own mtime. See `editor_get_capabilities`' `colorist_grade` block for the model. |
...
> that stack** (D-216, D-218, D-232, D-280), and none is an exception to D-140 so much as outside
...
## Motion tab — CLOSED, 37 tools (D-257, 2026-09-09; +4 D-259; +1 D-260)
...
> It is closed. Count:
> `grep -c '^def motion_' mcp/server.py` returns **37** (D-257's 32, D-259's
> +4 delete/duplicate tools, D-260's `motion_get_edit_links`), and a
> cross-language check confirms those 37 Python wire names and the 37 op keys
> in `packages/motion/src/motionOps.ts` agree exactly.
>
...
| `motion_render` | The real Remotion render — **one video file PER SCENE** (D-180), never one combined video. Since D-260 it also REFRESHES any Edit clips already placed from that scene. See the note below on what actually happens to those files. |
| `motion_get_edit_links` (new, D-260) | What each scene feeds in the Edit tab: `rendered`, its pool item's `mediaId`/`sourcePath`, `clipCount`, and each clip's `track`/`clip`/`clipId`/`name` — the addressing the `editor_*` tools take, so the answer is actionable rather than merely readable. Call it to see what a `motion_render` will refresh, or to confirm what it did. `rendered: false` (this scene has no render in the pool) and a top-level `available: false` (no link data reached the tab at all — no project open) are deliberately different answers. |
...
> `@apelles/history` undo stack a human's does (D-140). **`motion_select`,
...
>
> **D-260 replaced this note's previous "there is no live link" paragraph.**
...
> `editor_swap_clip_media` after a re-render** — that would be the pre-D-260
...
> again. That explicit trigger is deliberate (a render is expensive; see D-260).
>
...
| `chroma_media_reprobe` (B-073/D-270, 2026-09-09) | `editor_reprobe_media` | the Sources panel's own "Re-probe" row action |
...
**D-280 added a third argument to that same transport**, and it is worth noting
...
Unlike D-210's pair these are **frontend** ops (`@apelles/debug`'s registry,
