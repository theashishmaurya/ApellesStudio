...
**Yes, and Chroma is unusually well-positioned for it** — because the hard part of a
...
is exactly the grading + relight + depth work Chroma already has or is building.
...
| **New background** — a still, a video plate, or **AI-generated** (a Flux/SDXL-class image model + prompt for the static case; the owner's ref looks generated) | new — a `chroma-ai` generate step, or "bring your own plate" |
| **Composite** — matte the subject over the BG | `chroma-compositor` (roadmap / architecture-lock) |
...
Established VFX believability techniques, each mapped to a Chroma capability:
...
1. **Light wrap** — the BG's light spills slightly onto the FG edge (matched temperature
   ...
   edge band. New, small (~a `chroma-grade` / `chroma-compositor` stage).
...
grade-adjacent. **v2/v3** — needs `chroma-compositor` first, and the relight puck work
