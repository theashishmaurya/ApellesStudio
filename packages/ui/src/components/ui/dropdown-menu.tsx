...
/**
 * shadcn `dropdown-menu` on Base UI `Menu` (D-042).
 *
 * --accent decision (D-042): Base UI's highlight state uses `data-highlighted`
 * and the shadcn source's `bg-accent` (= brand white here) is swapped for
 * `bg-muted` / `text-foreground` so a highlighted item is a subtle hover
 * surface, not the brand colour.
 */
    ...
    <MenuPrimitive.Portal>
      ...
      >
        <MenuPrimitive.Popup
          ...
          className={cn(menuPopupClass, 'w-(--anchor-width)', className)}
          {...props}
        />
      </MenuPrimitive.Positioner>
    </MenuPrimitive.Portal>
