...
interface PlayerProps {
  ...
  rate?: number;                          // PLAYBACK rate, 1 = normal (D-278) — omit → control hidden
...
- **`rate` is the TRANSPORT's playback rate** (D-278) — timeline seconds
...
popover. Added by D-278 — the package had no test rig before it.
