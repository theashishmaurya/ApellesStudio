 ...
 * (`app/src/Root.tsx` → `setOpenProject`, B-083/D-202), the same one-input
...
interface MediaPoolState {
   ...
   *  `selectProjectKey`), `null` when none is open (B-083/D-202). */
   ...
   *
   *  B-083/D-202 — the Sources panel used to fire the initial `refresh()`
...
/** B-083/D-202 — monotonic `refresh()` token. A project switch fires a refresh
