...
import { DEFAULT_FADE_CURVE, fadeCurveEval, fadePresetName, type Clip, type FadeCurve } from './timeline';
 ...
 *  the same problem (approximating a `FadeCurve` for ffmpeg), reused rather
...
export function dynamicZoomKeyframes(
  ...
  first: number,
  last: number,
  curve: FadeCurve = DEFAULT_FADE_CURVE,
): ClipKeyframe[] {
  ...
  // `fadePresetName` is the same exact-match the Inspector's own curve
  ...
  const segments = fadePresetName(curve) === 'linear' ? 1 : DYNAMIC_ZOOM_EASE_SAMPLES;
  ...
  for (let i = 0; i <= segments; i++) {
    ...
    const framing = i === 0 ? start : i === segments ? end : lerpFraming(start, end, fadeCurveEval(curve, x));
