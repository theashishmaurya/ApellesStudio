...
export function keyframeExprAt(
  ...
  staticValue: number,
  fps: number,
  /** D-209 — which ffmpeg time variable the expression is written in terms
   *  of. `'t'` (the default, and every pre-D-209 caller) is right for a
...
// --------------------------------------------------------------------------- //
// Text / title clips (D-209/D-211) — compiled to ffmpeg's own `drawtext`
// --------------------------------------------------------------------------- //

/**
 * Quote and escape `raw` so ffmpeg delivers it to a filter option **verbatim**
 * — returns the value *including* its surrounding quotes (D-211).
 *
 ...
 * (D-211). A text clip has no media to open, so it has no ffmpeg input and no
 ...
 * rasterised** (D-210). Without it there is nothing honest to draw, so the
...
export interface TimelineExportOptions {
  ...
  /** D-209/D-210 — absolute font-file paths keyed by `TextLayer.font`'s
...
/**
 * D-209/D-210 — every distinct `TextLayer.font` key on `timeline` that
...
export function buildExportFfmpegArgs(timeline: Timeline, outPath: string, opts: TimelineExportOptions): string[] {
  ...
  interface PendingClip {
    clip: Clip;
    /** `null` for a TEXT clip (D-209): it opens no file, so it consumes no
  ...
  for (const { track, index: trackIndex } of paintOrder) {
    for (const clip of track.clips) {
      ...
      // D-209 — a TEXT clip has no file to open: no `-i`, no input index, no
  ...
  for (const p of pending) {
    // D-209 — a text clip has no per-clip chain to build (its `drawtext` node
  ...
  chains.forEach((chain, i) => {
    ...
    // D-209 — a text clip paints with `drawtext` on the stream built so far,
  ...
  for (const p of pending) {
    // D-209 — a text clip has no input stream at all, so `[N:a]` would not
