 ...
 *
 *  **And the default span of a STILL IMAGE clip (D-292)**, which is the case
...
/** D-292 — the still-image extensions the Edit tab accepts as a clip source.
 *
...
/** D-292 — whether `sourcePath` names a still image.
 *
...
/** D-292 — the synthesized SOURCE extent of a still, in seconds: the ceiling a
...
/** D-292 — the `source_len`/`source_fps`/default `duration` a clip built from a
 *  pool item gets, for a still and for moving footage alike.
 *
 *  **The one place that arithmetic lives.** Before D-292 it was spelled inline
...
export function mediaSourceFacts(
  ...
  return {
    ...
    // Moving footage is placed at its full length, unchanged from pre-D-292.
...
export function checkAddClip(
  ...
  // D-292 — a STILL is picture too, for the same reason and with one extra
 ...
 *
 *  D-292 — `fps` is the PROJECT's own rate (`timelineFps(tl)`), needed because
...
export function linkedClipsFromDraggedMedia(
  ...
  // D-292 — a still has no audio stream to link a half from, and
