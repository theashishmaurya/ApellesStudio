...
export interface Clip {
  ...
  /** Text/title layer (D-209) — mirrors `chroma_timeline::Clip::text`.
   ...
   *  which is every clip in every pre-D-209 project.
   *
   ...
   *  the Rust field's own doc and D-209 for the full comparison.
   *
...
/** A generated text/title layer (D-209) — mirrors `chroma_timeline::TextLayer`
 *  field for field.
 *
...
export interface TextLayer {
   ...
   *  See the Rust type's own doc and D-211. */
  content: string;
 ...
 *  (D-209). Mirrors `chroma_timeline::Clip::is_text` — one predicate, asked
...
export function newTextLayer(
  ...
  if (/[\r\n]/.test(content)) {
    return { error: 'content must be a single line — multi-line titles are not supported yet (D-211)' };
  }
 ...
 *  op (D-209) — a text clip is placed by exactly the same op a media clip is,
...
export type EditOp =
  ...
  /** D-209 — edit a TEXT clip's own text properties (content/font/size/
