...
function buildRow(tl: Timeline, fps: number): TimelineRow {
  ...
  // D-058: position comes directly from the clip's own `start_frame` (real,
...
export function TimelinePane() {
  ...
  const doSplit = () => {
    const at = playhead;
    // D-058: look up by real `start_frame`, not summed preceding durations —
      ...
      <div ref={editAreaRef} className="relative flex-1 min-h-0 overflow-hidden">
        ...
        <TimelineEditor
          ...
          onActionMoveEnd={({ action, start }) => {
            // D-058: a clip-body drag repositions it (`move`, overlap-
