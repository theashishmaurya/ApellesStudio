  ...
  return {
    ...
    tracks: tl.tracks.map((t, ti) => ({
      ...
      clips: t.clips.map((c, ci) => ({
        ...
        // D-209 — `null` for an ordinary media clip; the whole text layer for
  ...
  useEffect(() => {
    const OPS: Record<string, (args: any) => any> = {
      ...
      editor_set_clip_transform: (a) => {
        ...
        const c = found.c;
        // D-209 — a TEXT clip only honours opacity and position, in BOTH
    ...
    // D-209/D-210 — warm the font catalogue once, at Edit-tab mount. The
