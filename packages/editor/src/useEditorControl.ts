...
// D-278 — the transport's own rate bounds, from the package that owns the
    ...
    const OPS: Record<string, (args: any) => any> = {
      ...
      editor_get_state: () => {
        ...
        return {
          ...
          // D-278 — the transport's PLAYBACK rate, the read half of
      ...
      editor_set_playing: (a) => {
        ...
        return {
          ok: true,
          playing: s.playing,
          // D-278 — said here, not just in `editor_get_state`, because it
      ...
      // ---- D-278: the transport's playback RATE ---------------------------
      //
