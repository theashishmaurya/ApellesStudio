...
export function EditorInspectorPanel() {
  ...
  // `?? FADE_PRESETS[0].name` covers the disarmed case (nothing to name) and
  // the theoretical custom curve an MCP caller could arm with; `linear` is
  // what both mean to this picker.
  const dynamicZoomEasePreset = (dynamicZoomArmed ? fadePresetName(dynamicZoom?.curve) : null) ?? FADE_PRESETS[0].name;

  const armDynamicZoom = (armed: boolean) => {
    if (!selectedClip) return;
    setDynamicZoom(armed ? { clipId: selectedClip.id, curve: dynamicZoom?.curve ?? DEFAULT_FADE_CURVE } : null);
  };
  ...
  const setDynamicZoomEase = (curve: FadeCurve) => {
    if (!selectedClip) return;
