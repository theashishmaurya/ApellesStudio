 ...
 *
 * D-205 — and it is where the per-property keyframe model lives: this file
...
export function EditorInspectorPanel() {
  ...
  // D-205 — the `<` / `>` per-property keyframe nav moves the playhead.
  ...
  // D-205 — every keyframeable property's static (un-animated) value, read
  ...
  // D-205 — one write of a `set_clip_keyframes` op, so every per-property
  ...
  // D-205 — what `ClipInspectorPanel` renders each row from. `value` is the
  ...
  /** D-205 — edit ONE property's value.
   *
  ...
  /** D-205 — After Effects' stopwatch, for one property.
   *
  ...
  /** D-205 — jump the playhead to this property's own previous/next
  ...
  /** D-205 — put ONE property back to its rest value.
   *
  ...
  // D-205 — "Key all properties": the same MERGE every per-property write
  // uses, with all nine names at once, so it can never clobber a key a
  // single property's diamond already put at this frame (the pre-D-205
