...
// The first three below pass even against the pre-D-281 generic `-ss`/`-t`
...
describe.skipIf(!AVAILABLE)('a still image clip — real ffmpeg render (D-281)', () => {
  let dir: string;
  let out: string;
  ...
  /** **The bug this whole test exists for.** Before D-281 there was no way to
