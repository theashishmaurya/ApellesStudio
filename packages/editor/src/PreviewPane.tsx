 ...
 *
 * Playback RATE (D-278, roadmap "A preview PLAYBACK-RATE control"): the
 ...
 *     or muted. See that module and D-278 for why it is WSOLA here when a
...
export function PreviewPane() {
  ...
  // D-278 — the transport's playback RATE. Store state for the same reason
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      // D-278 — the playback rate is exactly one multiplication, here. The
    ...
    //
    // D-278 — `playbackRate` re-runs this effect on purpose, which RE-BASELINES
  ...
  useEffect(() => {
    ...
    const startFrame = useEditorTimelineStore.getState().playhead;
    // D-278 — `rate` makes the audio session run the timeline this many times
    ...
    // D-278 — `playbackRate` joins `playing`/`hasTimeline` deliberately: it is
  ...
  return (
    ...
    <div ref={fullscreenRef} className="relative flex min-h-0 flex-1 flex-col bg-bg-primary">
      ...
      <Player
        ...
        // D-278 — the transport's playback rate. `setPlaybackRate` clamps, so
