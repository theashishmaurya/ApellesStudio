...
- `motionProjectStore.ts` — **readiness** (B-058/D-150): `openProjectPath` —
  *which* project is open, pushed in from the composition root
  (`app/src/Root.tsx`; B-083/D-203 made it the project's identity rather than a
...
added. D-255 deliberately declined to ship an MCP-only deletion (that would
...
**Closed by D-255 (2026-09-09): 32 `motion_*` tools.** The op registry lives
